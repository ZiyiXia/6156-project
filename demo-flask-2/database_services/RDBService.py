...
def get_by_prefix(db_schema, table_name, column_name, value_prefix):
    ...
    return res
def add_by_prefix(db_schema, table_name, ID, nameFirst, nameLast, email, salary):
    print("1111")
    conn = _get_db_connection()
    cur = conn.cursor()
...
def update_attibute2_by_attribute1(db_schema, table1, table2, attribute1, attribute2, reference1, reference2, id, salary):
    conn = _get_db_connection()
    cur = conn.cursor()
    # attribute1: Students.id
    # attribute2: Professors.salary
    # table1: Students
    # table2: Professors
    # reference1: Students.professor_id
    # reference2: Professors.ID
    sql = "update " + db_schema + "." + table2 + " set " + attribute2 + " = " + salary + " where " + reference2 + " = (" \
